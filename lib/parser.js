    ...
    if(line==='next'){
      if(co&&cs){
        if(cs==='firewall policy'){
          ...
          if(co.start&&co.end){const sa=Array.isArray(co.start)?co.start:[co.start];const ea=Array.isArray(co.end)?co.end:[co.end];if(sa.length>=2&&ea.length>=2)acc.schedules[ce]={start:`${String(sa[1]).replace(/\//g,'-')}T${sa[0]}`,end:`${String(ea[1]).replace(/\//g,'-')}T${ea[0]}`};}
        }
      }
...
const parsePAConfig = (text) => {
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(schedM){const sn=String(schedM[1]).replace(/['"]/g,'');acc.schedules[sn]={start:`${schedM[2].replace(/\//g,'-')}T${schedM[3]}`,end:`${schedM[4].replace(/\//g,'-')}T${schedM[5]}`};continue;}
    // Routes
