...
const parseFortiGateConfig = (text) => {
  ...
  const acc={type:'FortiGate',hostname:'Unknown-FortiGate',tags:{},addresses:{},groups:{},services:{},serviceGroups:{},urlCategories:{},secRules:{},natRules:{},routes:{},schedules:{},unparsed:[]};
  ...
  const secRules=Object.values(acc.secRules).map(r=>({name:String(r.name),disabled:r.disabled,action:r.action,from:toArr(r.from),to:toArr(r.to),source:toArr(r.source),destination:toArr(r.destination),application:toArr(r.application),service:toArr(r.service),category:toArr(r.category),tag:toArr(r.tag),schedule:r.schedule||null}));
