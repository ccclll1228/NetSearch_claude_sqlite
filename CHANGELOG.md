...
- **LTM row toggle vs. text selection** — selecting text in an LTM virtual server row (VS name, destination IP, port, pool) no longer accidentally expands/collapses the row. The row toggle is now guarded by a drag-distance check (skips if the pointer moved >4px between mousedown and click) and a selection check (skips when `window.getSelection()` is non-empty); the ▶ expand-arrow gets its own dedicated click zone that toggles unconditionally so expand/collapse is never broken

### Changed
- Ignore CIDR toggle now defaults to ON — CIDR containment checks are skipped unless explicitly disabled

### Added
- **Raw Config tab line filter** — an Enter-triggered filter bar atop the Raw Config tab shows only config lines containing the keyword (case-insensitive, plain text, line-level show-only-matching). Device cards with zero matches are hidden, the matched keyword is highlighted with a warm `<mark>`, and a `N devices · M lines` counter is shown while active. Includes a clear (×) button; clearing/empty restores the full view and switching tabs resets the filter. Purely additive — the original render path is unchanged when no filter is active
- **Raw Config filter — context lines toggle** — a grep -C style `± Context` toggle in the filter bar (with a custom N input, default 2, range 0–20) shows N lines above and below each match. Overlapping/adjacent ranges merge into continuous blocks and non-adjacent blocks are separated by a muted `···` divider; matching lines keep the highlight while context lines render plain, and the `N devices · M lines` counter still counts matching lines only. Changing N re-runs the filter immediately; the toggle state and count persist across tab switches (only the filter term resets). Bilingual labels/tooltip. Additive — with context OFF the output is identical to the plain line filter
